...
class Runner(Process):
    ...
    def _calc_ect(self, task: Message) -> int:
        stct: int = ESTCT[task["type"]][MODELS[task["taskType"]]]
        return stct * task["dataSize"]

    def _update_ect(self, mode: str = "", task: Message = None) -> None:
        if task:
            if mode == "append":
    ...
    def _manage_task(self, task: Message) -> None:
        for attempt in range(self._max_retries):
            try:
                ...
                if self._mode == "gpu":
                    ...
                    output: object = self._execute_task(task, model_summary)
                else:
                    logger.debug(
    ...
    def _execute_task(
        self, task: Message, model_summary: ModelSummary
    ) -> object:
        """Executes a task."""
